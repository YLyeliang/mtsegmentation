# -*- coding: utf-8 -*- 
# @Time : 2020/11/9 2:29 下午 
# @Author : yl
import torch.nn as nn
from mtcv.cnn import build_conv_layer, build_norm_layer, constant_init, kaiming_init
from mtcv.runner import load_checkpoint
from mtcv.utils.parrots_wrapper import _BatchNorm

from seg.ops import Upsample, resize
...
class HRModule(nn.Module):
    ...
    def _check_branches(self, num_branches, num_blocks, in_channels, num_channels):
        ...
        if num_branches != len(num_blocks):
            err = f'NUM_BRANCHES({num_branches}) <> NUM_BLOCKS(' \
                  f'{len(num_blocks)})'
            raise ValueError(err)

        if num_branches != len(num_channels):
            err = f'NUM_BRANCHES({num_branches}) <> NUM_CHANNELS(' \
                  f'{len(num_channels)})'
            raise ValueError(err)

        if num_branches != len(in_channels):
            err = f'NUM_BRANCHES({num_branches}) <> NUM_INCHANNELS(' \
                  f'{len(in_channels)})'
            raise ValueError(err)
                ...
                if j > i:
                    fuse_layer.append(
                        nn.Sequential(
                            ...
                            # Set align_corners=False for HRNet
                            Upsample(
                                scale_factor=2 ** (j - i),
                                mode='bilinear',
                                align_corners=False)))
...
class HRNet(nn.Module):
    ...
    def train(self, mode=True):
        """Convert the model into training mode while keeping the normalization layer frozen"""
