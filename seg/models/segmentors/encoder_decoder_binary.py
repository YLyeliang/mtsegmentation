...
from seg.core import add_prefix
...
class EncoderDecoderbinary(BaseSegmentor):
    ...
    def __init__(self,
                 ...
                 test_cfg=None,
                 pretrained=None):
        super(EncoderDecoderbinary, self).__init__()
    ...
    def init_weights(self, pretrained=None):
        ...
        super(EncoderDecoderbinary, self).init_weights(pretrained)
    ...
    def inference(self, img, img_meta, rescale):
        ...
        Args:
            ...
            img_meta (dict): Image info dict where each dict has: 'img_shape',
                ...
                'filename', 'ori_shape', 'pad_shape', and 'img_norm_cfg'.
                For details on the values of these keys see
                `seg/datasets/pipelines/formatting.py:Collect`.
    ...
    def simple_test(self, img, img_meta, rescale=True, thresh=0.5):
        """Simple test with single image."""
        if self.test_cfg.thresh is not None:
            thresh = self.test_cfg.thresh

        seg_logit = self.inference(img, img_meta, rescale)
        seg_pred = seg_logit.sigmoid()
        foreground = torch.ones_like(seg_pred)
        background = torch.zeros_like(seg_pred)
        seg_pred = torch.where(seg_pred > thresh, foreground, background)

        if torch.onnx.is_in_onnx_export():
            # inference backend only support 4D output
    ...
    def aug_test(self, imgs, img_metas, rescale=True, thresh=0.5):
        ...
        if self.test_cfg.thresh is not None:
            thresh = self.test_cfg.thresh
        # rescale all imgs back to ori_shape
        assert rescale
